% preprocess data
function preprocesssfudata
  
    params =  sfuparameters();
...
function preprocessdata1(params,DIRNUM,VLADvocab,BOWvocab,planes,c1,k,dim,bpw,wordnum,datadir)
    ...
    if DIRNUM == 1
        ...
        dd = params.d1;
        params.offsetdirn = params.offsetdirn1;
    elseif DIRNUM == 2
        dlength = params.dlength2;
        dd = params.d2;
        params.offsetdirn = params.offsetdirn2;
    else
...
function IM = loadimage(params,dname,jj)
       ...
       if params.offsetimage
            IM = offsetimage(IM,params.offset,params.offsetdirn);
       end
...
% 	Returns the signature bits for two data points.
 ...
 function I1 = offsetimage(IM,offset,dirn)

    ssz = size(IM);
    offrac = offset/100;
    of = ssz(1)*offrac;
    if dirn==1
        I1 = IM((of+1):ssz(1),1:ssz(2),:);
    elseif dirn==0
        I1 = IM(1:ssz(1)-ofo,1:ssz(2),:);
    else
        error('unknown dirn')
    end
end